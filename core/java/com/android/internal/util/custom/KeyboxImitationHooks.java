...
public class KeyboxImitationHooks {
    ...
    private static PrivateKey parsePrivateKey(String encodedKey, String algorithm)
        ...
        if(algorithm.equals("EC")){
            return ConvertECPrivateKey.convertECPrivateKey(encodedKey);
        }else{
            return ConvertRSAPrivateKey.convertRSAPrivateKey(encodedKey);
        }
    }
    ...
    private static byte[] getCertificateChain(String algorithm) throws Exception {
        ...
        if(algorithm.equals("EC")){
            certChain = KeyProviderManager.getEcCertificateChain();
        }
        else{
            certChain = KeyProviderManager.getRsaCertificateChain();
        }
    ...
    private static PrivateKey getPrivateKey(String algorithm) throws Exception {
        String privateKeyEncoded = null;
        if(algorithm.equals("EC")){
            privateKeyEncoded = KeyProviderManager.getEcPrivateKey();
        }
        else{
            privateKeyEncoded = KeyProviderManager.getRsaPrivateKey();
        }
    ...
    private static X509CertificateHolder getCertificateHolder(String algorithm) throws Exception {
        String certChain = null;
        if(algorithm.equals("EC")){
            certChain = KeyProviderManager.getEcCertificateChain()[0];
        }
        else{
            certChain = KeyProviderManager.getRsaCertificateChain()[0];
        }
